// choose rock, paper or scissors for computer => getComputerChoice();
  // [OK] make computer choose a random number from 0 to 2 => getComputerRandomNumber();
  // [OK] create rule to correlate each value to an option (rock, paper or scissors) => 0 is Paper, 1 is Rock, 2 is Scissors (alphabetical order)
...
// get choice from player => getPlayerChoice();
  ...
  // [OK]check if player made a correct input (rock, paper or scissors) => let validChoice;
    // [OK] if not, keep asking until they get it right => while loop, sentry invalidInput
...
// compare player and computer's choices
  // [OK]if equal, it's a tie
    // if tied, store NULL in a variable;
  // [OK]if different, apply a rule to determine the winner;
...
// create a function game() to play a 5-round game that keeps score and reports a winner at the end
...
function showScore(playerSelection, computerSelection, playerScore, computerScore, roundResults) {
  if(roundResults === true) {
...
function game() {
  ...
  while (keepGoing) {
    ...
    showScore(playerSelection, computerSelection, playerScore, computerScore, roundResults);
  }
